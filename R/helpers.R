...
#' Returns meta information of the dataframe like Number of variables, Number of observations, Total missing (%), Total size in memory
#'
...
get_skewed_vars <- function(df,skewness_threshold=skewness_threshold_pkg_default){
  ...
  skewness_vars <- sapply(df_numeric, function(x){skewness(x,na.rm=T)})
...
get_correlated_vars <- function(df,correlation_threshold=correlation_threshold,
                                method_name="pearson"){
  melted_cormat <- .get_cor_mat_private(melted_cormat,method=method_name)
...
.get_cor_mat_private <- function(df, method_name="pearson"){
  df_numeric <- df %>% Filter(f = is.numeric)
  cormat_pearson <- round(cor(df_numeric, use="pairwise.complete.obs", method=method_name),2)
  melted_cormat <- melt(cormat_pearson)
  melted_cormat
}
